    ...
    try:
        ...
        if zip_config:
            ...
            if not settings.DEBUG:
                message = {
                    "text": "A *new challenge* has been uploaded to EvaAI.",
                    "fields": [
                        {
                            "title": "Email",
            ...
            response_data = {
                ...
                " sent for review to EvaAI Admin.".format(challenge.title)
            }
            return Response(response_data, status=status.HTTP_201_CREATED)
...
def manage_worker(request, challenge_pk, action):
    ...
    # Only allow EvaAI admins to delete workers
        ...
        if not challenge_queryset:
            ...
            try:
                ...
                if zip_config:
                    ...
                    if not settings.DEBUG:
                        message = {
                            "text": "A *new challenge* has been created on EvaAI.",
                            "fields": [
                                {
                                    "title": "Email",
                                    "value": request.user.email,
                    ...
                    response_data = {
                        "Success": "Challenge {} has been created successfully and"
                        " sent for review to EvaAI Admin.".format(
                            challenge.title
                        )
                    }
                    return Response(
...
def pwc_task_dataset(request):
    if request.user.is_staff:
        ...
        response_data = {
            "error": "You are not authorized to make this request. Please ask EvaAI admin to add you as a staff user for accessing this API."
        }
        return Response(response_data, status=status.HTTP_406_NOT_ACCEPTABLE)
...
def request_challenge_approval_by_pk(request, challenge_pk):
    ...
    if not settings.DEBUG:
        try:
            EvaAI_api_server = settings.EvaAI_API_SERVER
            approval_webhook_url = settings.APPROVAL_WEBHOOK_URL

            if not EvaAI_api_server:
                raise ValueError("EvaAI_API_SERVER environment variable is missing.")
        ...
        message = {
            ...
            "fields": [
                {
                    "title": "Admin URL",
                    "value": f"{EvaAI_api_server}/api/admin/challenges/challenge/{challenge_pk}",
                    "short": False,
                },
                {
                    "title": "Challenge title",
