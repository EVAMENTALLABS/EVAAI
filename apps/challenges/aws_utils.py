...
EvaAI_DNS = os.environ.get("SERVICE_DNS")
aws_keys = {
    ...
    "AWS_STORAGE_BUCKET_NAME": os.environ.get(
        "AWS_STORAGE_BUCKET_NAME", "EvaAI-s3-bucket"
    ),
}
...
COMMON_SETTINGS_DICT = {
    "EXECUTION_ROLE_ARN": os.environ.get(
        "EXECUTION_ROLE_ARN",
        "arn:aws:iam::{}:role/EvaAITaskExecutionRole".format(
            aws_keys["AWS_ACCOUNT_ID"]
        ),
    ),
    "WORKER_IMAGE": os.environ.get(
        "WORKER_IMAGE",
        "{}.dkr.ecr.us-east-1.amazonaws.com/EvaAI-{}-worker:latest".format(
    ...
    "CLUSTER": os.environ.get("CLUSTER", "EvaAI-prod-cluster"),
    "DJANGO_SERVER": os.environ.get("DJANGO_SERVER", "localhost"),
    "EvaAI_API_SERVER": os.environ.get("EvaAI_API_SERVER", "localhost"),
    ...
    if prev != curr:
        ...
        if count != 1:
            ...
            challenge_url = "{}/web/challenges/challenge-page/{}".format(
                settings.EvaAI_API_SERVER, challenge.id
            )
...
def create_eks_nodegroup(challenge, cluster_name):
    """
    Creates a nodegroup when a EKS cluster is created by the EvaAI admin
    Arguments:
...
def setup_eks_cluster(challenge):
    ...
    environment_suffix = "{}-{}".format(challenge_obj.pk, settings.ENVIRONMENT)
    eks_role_name = "EvaAI-code-upload-eks-role-{}".format(environment_suffix)
    eks_arn_role = None
    try:
    ...
    node_group_role_name = "EvaAI-code-upload-nodegroup-role-{}".format(
        environment_suffix
    )
...
def create_eks_cluster_subnets(challenge):
    ...
    try:
        ...
        response = client.create_security_group(
            GroupName="EvaAI code upload challenge",
            Description="EvaAI code upload challenge worker group",
            VpcId=vpc_ids[0],
        )
        security_group_id = response["GroupId"]

        response = client.create_security_group(
            GroupName="EvaAI-code-upload-challenge-efs-{}".format(
                environment_suffix
            ),
...
def create_eks_cluster(challenge):
    """
    Called when Challenge is approved by the EvaAI admin
